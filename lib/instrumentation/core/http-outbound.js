...
module.exports = function instrumentOutbound(agent, opts, makeRequest) {
  ...
  function instrumentRequest(segment) {
    ...
    // FLAG: synthetics
    if (
      config.feature_flag.synthetics &&
      config.encoding_key &&
    ...
    // If CAT is enabled, inject the transaction header. Always inject the trace
    // context headers.
...
function _addTraceContextHeaders(tx, outboundHeaders) {
  try {
    ...
    if (!outboundHeaders[TRACE_CONTEXT_PARENT_HEADER]) {
      outboundHeaders[TRACE_CONTEXT_PARENT_HEADER] = traceContext.parent
    }
    if (!outboundHeaders[TRACE_CONTEXT_STATE_HEADER]) {
      outboundHeaders[TRACE_CONTEXT_STATE_HEADER] = traceContext.state
    }
