...
module.exports = function instrumentOutbound(agent, opts, makeRequest) {
  ...
  function instrumentRequest(segment) {
    ...
    if (agent.config.cross_application_tracer.enabled) {
      ...
      } else if (agent.config.feature_flag.trace_context) {
        const traceParent = opts.headers[TRACE_CONTEXT_PARENT_HEADER]
        const traceState = opts.headers[TRACE_CONTEXT_STATE_HEADER]
