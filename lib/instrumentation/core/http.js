...
const TRACE_CONTEXT_STATE = 'tracestate'
const TRACE_CONTEXT_PARENT = 'traceparent'
...
function wrapEmitWithTransaction(agent, emit, isHTTPS) {
  ...
  return tracer.transactionProxy(function wrappedHandler(evnt, request, response) {
    ...
    if (agent.config.cross_application_tracer.enabled) {
      const parent = request.headers[TRACE_CONTEXT_PARENT]
      const state = request.headers[TRACE_CONTEXT_STATE]
      if (parent && state) {
        logger.trace(
          'Accepting distributed trace payload for transaction %s',
          transaction.id
        )
        transaction.acceptDistributedTracePayload(parent, state, transport)
      }
    }
...
function wrapRequest(agent, request) {
  return function wrappedRequest(options) {
    ...
    if (agent.config.cross_application_tracer.enabled) {
      const traceParent = options.headers[TRACE_CONTEXT_PARENT]
      const traceState = options.headers[TRACE_CONTEXT_STATE]
      transaction.storeTraceContext(traceParent, traceState)
...
function _addDistributedHeaders(tx, outboundHeaders) {
  try {
    const traceContext = tx.createDistributedTraceHeaders({
      format: 'http',
      isBinary: true
    })
    outboundHeaders[TRACE_CONTEXT_PARENT] = traceContext.parent
    outboundHeaders[TRACE_CONTEXT_STATE] = traceContext.state
