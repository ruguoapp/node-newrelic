...
// This global is used to track the timer used for ending transactions
let lambdaTimeoutTimer = null
  ...
  patchLambdaHandler(handler) {
    ...
    function wrappedHandler() {
      ...
      if (context.getRemainingTimeInMillis) {
        // In the case of the lambda timing out, we won't be notified in
        // order to end the transaction. We schedule the end of the
        // transaction, and clear it before the time out occurs.
        const serializationTime = 50 // in milliseconds
        const timeLeft = context.getRemainingTimeInMillis()
        if (timeLeft > serializationTime) {
          lambdaTimeoutTimer = setTimeout(txnEnder, timeLeft - serializationTime)
          lambdaTimeoutTimer.unref()
        }
      } else {
        logger.debug(
          'getRemainingTimeInMillis was missing, not registering timeout ender'
        )
      }
...
function endTransaction(transaction, enderIndex) {
  ...
  if (lambdaTimeoutTimer) {
    clearTimeout(lambdaTimeoutTimer)
    lambdaTimeoutTimer = null
  }
