...
const DTPayload = require('./ts-payload')
...
function acceptDistributedTracePayload(parent, state, transport) {
  if (!parent || !state) {
  ...
  const parsed = this._getParsedPayload(state)
  if (!parsed) {
    return
  }
 ...
 */
Transaction.prototype._getParsedPayload = function _getParsedPayload(state) {
  const matched = state.match(TRACE_STATE_PAYLOAD_REGEX)
  const payload = matched[1] != null ? matched[1] : matched[2]
  ...
  if (typeof payload === 'string') {
    ...
    try {
      parsed = DTPayload.parseBinary(payload)
    } catch (err) {
      logger.warn(
        err,
 ...
 */
Transaction.prototype.createDistributedTraceHeaders = createDistributedTraceHeaders

function createDistributedTraceHeaders(options) {
  if (this.traceContext.state) {
    // TODO: would we want to rearrange the existing headers,
    // since nr= is not necessarily the first element on acceptance?
  ...
  let payload = new DTPayload(data)
  ...
  const state = this.traceContext.state
    ? `${nrState},${this.traceContext.state}`
    : nrState
