...
const TRACE_STATE_PAYLOAD_REGEX = /,\s*nr=([^,\$]+)|^\s*nr=([^,\$]+)(?:,)?/
...
function acceptTraceContextPayload(parent, state, transport) {
  ...
  let payload = null
  state = state.replace(TRACE_STATE_PAYLOAD_REGEX, (match, p1, p2) => {
    payload = p1 || p2
    return ''
  })
...
function createTraceContextHeaders(options) {
  ...
  const data = traceContextEnabled && this._createTracePayload()
  if (!data) {
  ...
  const parent = `00-${data.tr}` // TODO: span id & trace options?
  const nrState = `nr=${payload}`
  const state = this.traceContext.state
    ? `${nrState},${this.traceContext.state.replace(TRACE_STATE_PAYLOAD_REGEX, '')}`
    : nrState
