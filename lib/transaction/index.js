...
const DistributedTracePayload = require('./dt-payload')
const DistributedTracePayloadStub = DistributedTracePayload.Stub
const TraceStatePayload = require('./ts-payload')
...
const TRACE_STATE_PAYLOAD_REGEX = /(?:^|,)\s*nr=([^,]+)/g
...
Transaction.prototype.getIntrinsicAttributes = function getIntrinsicAttributes() {
  if (!this._intrinsicAttributes.totalTime) {
    ...
    if (config.cross_application_tracer.enabled) {
      if (config.distributed_tracing.enabled || config.feature_flag.trace_context) {
...
function acceptDistributedTracePayload(payload, transport) {
  ...
  // TODO: so far not doing any verification with parent on accept
  const parsed = this._getParsedPayload(payload, false) // ASCII JSON payload
  if (parsed) {
    this._acceptTracePayload(parsed, transport)
  }
}

Transaction.prototype.acceptTraceContextPayload = acceptTraceContextPayload
function acceptTraceContextPayload(parent, state, transport) {
  if (!parent || !state) {
    this.agent.recordSupportability('DistributedTrace/AcceptPayload/Ignored/Null')
    return
  }
  ...
  // If we're disabled, we'll be passing the headers through unadulterated.
  if (!this.agent.config.feature_flag.trace_context) {
    this.traceContext.state = state
    return
  }

  let payload = null
  state = state.replace(TRACE_STATE_PAYLOAD_REGEX, (match, $1) => {
    payload = $1
    return ''
  })

  // store remaining tracestate header
  this.traceContext.state = state

  if (!payload) {
    // No nr field in tracestate
    return
  }

  // TODO: so far not doing any verification with parent on accept
  const parsed = this._getParsedPayload(payload, true) // Binary payload
  if (parsed) {
    this._acceptTracePayload(parsed, transport)
  }
}

Transaction.prototype._acceptTracePayload = _acceptTracePayload
function _acceptTracePayload(parsed, transport) {
 ...
 */
Transaction.prototype._getParsedPayload = function _getParsedPayload(payload, binary) {
  let parsed = payload

  if (typeof payload === 'string') {
    if (payload.charAt(0) !== '{' && payload.charAt(0) !== '[') {
      try {
        payload = hashes.makeBuffer(payload, 'base64')
      } catch (err) {
        logger.warn(
          err,
    ...
    try {
      parsed = binary ? TraceStatePayload.parseBinary(payload) : JSON.parse(payload)
    } catch (err) {
      logger.warn(
        err,
...
function createDistributedTracePayload() {
  const distTraceEnabled = this.agent.config.distributed_tracing.enabled
  if (!distTraceEnabled) {
    logger.warn('Distributed tracing disabled for %s', this.id)
    return new DistributedTracePayloadStub()
  }

  const data = this._createTracePayload()
  return data ? new DistributedTracePayload(data) : new DistributedTracePayloadStub()
}

Transaction.prototype.createTraceContextHeaders = createTraceContextHeaders
function createTraceContextHeaders(options) {
  options = options || Object.create(null)
  const traceContextEnabled = this.agent.config.feature_flag.trace_context
  if (traceContextEnabled) {
    this._calculatePriority()
  }

  const data = traceContextEnabled && this.sampled && this._createTracePayload()
  if (!data) {
    logger.warn('Trace context disabled for %s', this.id)
    return {
      parent: this.traceContext.parent || '',
      state: this.traceContext.state || ''
    }
  }

  let payload = new TraceStatePayload(data)

  if (options.format === 'http') {
    payload = payload.httpSafe(options.isBinary)
  } else if (options.format === 'text') {
    payload = payload.text(options.isBinary)
  }

  let traceId = ''
  const spanId = strings.leftPad(data.id, 16, '0')
  const traceOpts = this.sampled ? '01' : '00'

  const oldParent = this.traceContext.parent
  if (oldParent && (oldParent.startsWith('00-') || /^..-.{32}-/.test(oldParent))) {
    traceId = oldParent.substr(3, 32)
  } else {
    traceId = strings.leftPad(hashes.makeId() + hashes.makeId(), 32, '0')
  }

  const parent = `00-${traceId}-${spanId}-${traceOpts}`
  let state = `nr=${payload}`
  if (this.traceContext.state) {
    if (this.traceContext.state[0] !== ',') {
      state += ','
    }
    state += this.traceContext.state
  }

  this.storeTraceContext(parent, state)

  return {parent, state}
}

Transaction.prototype._createTracePayload = function _createTracePayload() {
  const config = this.agent.config
  const accountId = config.account_id
  const appId = config.application_id
  const catEnabled = config.cross_application_tracer.enabled

  if (!accountId || !appId || !catEnabled) {
    logger.warn(
...
Transaction.prototype.storeTraceContext = function storeTraceContext(parent, state) {
  this.traceContext.parent = parent || this.traceContext.parent
  this.traceContext.state = state || this.traceContext.state
}
