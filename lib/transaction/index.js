...
const TRACE_STATE_PAYLOAD_REGEX = /(?:^|\,)nr=([^,]+)(?:\,|$)/
...
function acceptDistributedTracePayload(parent, state, transport) {
  ...
  const parsed = this._getParsedPayload(state) // TODO: so far not doing anything with parent
...
Transaction.prototype._getParsedPayload = function _getParsedPayload(state) {
  let payload
  state = state.replace(TRACE_STATE_PAYLOAD_REGEX, (match, p1) => {
    payload = p1
    return ''
  })

  // store whatever is left in the tracestate header
...
function createDistributedTraceHeaders(options) {
  ...
  const state = `nr=${payload},${this.traceContext}`
