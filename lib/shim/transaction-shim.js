...
function handleCATHeaders(headers, segment, transportType) {
  ...
  const state = headers[TRACE_CONTEXT_STATE_HEADER]
  if (parent && state) {
    tx.acceptTraceContextPayload(parent, state, transportType || 'Unknown')
  }
  ...
  if (catId && transactionData && config.encoding_key) {
    cat.handleCatHeaders(catId, transactionData, config.encoding_key, tx)
    if (tx.incomingCatId) {
      this.logger.trace(
...
function insertCATRequestHeaders(headers, useAlternateHeaderNames) {
  ...
  var usingDistributedTracing = (
    config.distributed_tracing.enabled && !config.feature_flag.trace_context
  )
  if (!config.encoding_key && !usingDistributedTracing) {
  ...
  if (!headers[TRACE_CONTEXT_STATE_HEADER]) {
    headers[TRACE_CONTEXT_STATE_HEADER] = traceContext.state
  }

  // Do other cross-application tracing too.
