...
function handleCATHeaders(headers, segment, transportType) {
  ...
  const parent = headers[TRACE_CONTEXT_PARENT]
  const state = headers[TRACE_CONTEXT_STATE]
  if (parent && state) {
    tx.acceptDistributedTracePayload(parent, state, transportType || 'Unknown')
  }
...
function insertCATRequestHeaders(headers, useAlternateHeaderNames) {
  ...
  headers[TRACE_CONTEXT_PARENT] = traceContext.parent
  headers[TRACE_CONTEXT_STATE] = traceContext.state

  // Determine the names of the headers we'll add.
  const transHeader = HTTP_CAT_TRANSACTION_HEADER
